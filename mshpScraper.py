...
class mshpScraper:
    ...
    def __call__(self, refreshCache = False):
        ...
        _dataset['Datetime'] = [datetime.datetime.combine(parser.parse(_dataset['Date'][row]).date(), parser.parse(_dataset['Time'][row]).time()) for row in range(len(_dataset['Date']))]        #Formatted test print
